		...
		<div class="easyui-layout" id="subWrap" fit="true" style="width:100%;height:100%;">
			...
			<form action="#" onsubmit="return submitQuery();" id="queryFormId"
				...
				<table cellpadding="1">
					<tr>
						<td>申请人:</td>
						<td><input class="easyui-textbox" name="applicant_like"
					...
					<tr>
						...
						<td><input class="easyui-datetimebox" name="applyTimeAfter"
							...
							editable="false" /></td>
						<td>到</td>
						<td><input class="easyui-datetimebox" name="applyTimeBefore"
							style="width: 250px; height: 26px;" editable="false"
							editable="false" /></td>
					</tr>
					<tr>
						<td>数据状态:</td>
						<td><select class="easyui-combobox" name="dataState_equ"
					...
					<tr>
						<td>工单类别:</td>
						<td><select class="easyui-combobox" name="orCategory_equ"
							style="width: 250px; height: 26px;">
								<option value=''></option>
								<option value='test1'>test1</option>
								<option value='test2'>test2</option>
...
function submitQuery(){
	doing();
	$('#list_data').datagrid({
		url : '<%=basePath%>/itsmorder/queryItsmOrderDo.do?'+$('form').serialize()
	});
...
$(function(){
	$('#list_data').datagrid({
		...
		columns:[[
				  ...
				  {field:'orId',title:'<span>工单号</span>',align:'center',sortable:'true',width:100},
		          	{field:'orCategory',title:'<span>工单类别</span>',align:'center',sortable:'true',width:100},
		          	{field:'title',title:'<span>标题</span>',align:'center',sortable:'true',width:100},
		          	{field:'workContent',title:'<span>作业内容</span>',align:'center',sortable:'true',width:100},
		          	{field:'applicant',title:'<span>申请人</span>',align:'center',sortable:'true',width:100},
		          	{field:'applyTime',title:'<span>申请时间</span>',align:'center',sortable:'true',width:100},
		          	{field:'performer',title:'<span>实施人</span>',align:'center',sortable:'true',width:100},
		          	{field:'performTime',title:'<span>实施时间</span>',align:'center',sortable:'true',width:100},
		          	{field:'isowner',title:'<span>IS担当</span>',align:'center',sortable:'true',width:100},
		          	{field:'businessSystem',title:'<span>业务系统</span>',align:'center',sortable:'true',width:100},
		          	{field:'performMode',title:'<span>实施方式</span>',align:'center',sortable:'true',width:100},
		          	{field:'annotation',title:'<span>注解</span>',align:'center',sortable:'true',width:100},
		          	{field:'dataState',title:'<span>数据状态</span>',align:'center',sortable:'true',width:100},
...
// 	              {field:'att3',title:'<span>扩展属性3</span>',align:'center',width:100},
			...
			handler: function(){
				...
				if(rowObj.length == 0){
					...
					$.messager.confirm('提示框', '你确定要删除吗?',function(r){
						if (r) {
							$.ajax({
								'url':'<%=basePath%>/itsmorder/bacthDeleteItsmOrderDo.do',
								'type':'post',
								'data':{'uuids':uuids},
								'dateType':'xml',
	...
	function openEdit(uuid){
		$('#editIframe').attr("src","<%=basePath%>/itsmorder/fowardEditItsmOrder.do?uuid_equ="+ uuid);
